...
write.csv(TweetsDF,file = "dhoni.csv")
write.table(TweetsDF,file="dhoni.txt")
getwd()
##############################################
library(rJava)
...
x = readLines('D:/WORK/TEXT MINING/dhoni.txt') 	# first, read-in data from "iphone reviews.txt"
...
##
dhoni_tweet <- read.csv("D:/WORK/TEXT MINING/dhoni.csv")
dhoni <- iconv(dhoni_tweet$text)
View(dhoni)
s <- get_nrc_sentiment(dhoni)
head(s)
##barplot
barplot(colSums(s),
        las= 2,
        col= rainbow(10),
        ylab = "count",
        main= " SENTIMENT SCORES FOR dhoni REVIEWS")
#From above sentiment analysis of the 
##amazon reviews of the dhoni  are positive and trust based comments.. 
