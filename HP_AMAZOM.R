...
library(magrittr)
# Amazon Reviews #############################
aurl <-"https://www.amazon.in/HP-da1058tu-15-6-inch-Integrated-Sparkling/dp/B07TD1SCY6/ref=sr_1_1?crid=3BQXPQOJ2VP5U&dchild=1&keywords=hp+15-da1058tu+laptop&qid=1587300548&s=electronics&sprefix=hp+15-da1058TU%2Celectronics%2C524&sr=1-1#customerReviews"
amazon_reviews <- NULL
for (i in 1:20){
...
write.table(amazon_reviews,file = "hp.txt")
write.csv(amazon_reviews,file ="hp.csv")
getwd()
##############################################
library(rJava)
...
pos.words=scan('D:/WORK/TEXT MINING/positive-words.txt', what="character", comment.char=";")	# read-in positive-words.txt
neg.words=scan('D:/WORK/TEXT MINING/negative-words.txt', what="character", comment.char=";") 	# read-in negative-words.txt
pos.words=c(pos.words,"wow", "kudos", "hurray") 			# including our own positive words to the existing list
neg.words = c(neg.words)
stopwdrds = readLines('D:/WORK/TEXT MINING/stop.txt')
# Loading the text file to be analysed 
x = readLines('D:/WORK/TEXT MINING/hp.txt') 	# first, read-in data from "iphone reviews.txt"
...
##
hp_review <- read.csv("D:/WORK/TEXT MINING/hp.csv")
hp <- iconv(hp_review$x)
View(hp)
s <- get_nrc_sentiment(hp)
head(s)
##barplot
barplot(colSums(s),
        las= 2,
        col= rainbow(10),
        ylab = "count",
        main= " SENTIMENT SCORES FOR hp REVIEWS")
#From above sentiment analysis of the 
##amazon reviews of the hp notebook laptop are positive and trust based comments.. 
##And also anticipation comments..
