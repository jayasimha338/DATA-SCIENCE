...
a<-10
kathi <-NULL
url1<-"https://www.imdb.com/title/tt9900782/?pf_rd_m=A2FGELUUNOQJNL&pf_rd_p=f0294795-5981-40c8-b642-15ce8ebdecf6&pf_rd_r=DZ6CTRV04J12068KK16A&pf_rd_s=center-1&pf_rd_t=60601&pf_rd_i=india.toprated&ref_=fea_india_ss_toprated_india_tr_in_i_2"
for(i in 0:6){
  url<-read_html(as.character(paste(url1,i*a,sep="")))
  kathi1<-url %>%
    html_nodes("p") %>%
    html_text() 
  kathi <-c(kathi,kathi1)
}
write.table(kathi,file="kathi.txt")
write.csv(kathi,file = "kathi.csv")
getwd()
##############################################
library(rJava)
...
x = readLines('D:/WORK/TEXT MINING/kathi.txt') 	# first, read-in data from "iphone reviews.txt"
...
##
kathi_review <- read.csv("D:/WORK/TEXT MINING/kathi.csv")
kathi1 <- iconv(kathi_review$x)
View(kathi1)
s <- get_nrc_sentiment(kathi1)
head(s)
##barplot
barplot(colSums(s),
        las= 2,
        col= rainbow(10),
        ylab = "count",
        main= " SENTIMENT SCORES FOR kathi1 REVIEWS")
#From above sentiment analysis of the 
##Reviews of the kathi are positive and trust based comments...
##And also negative comments..
